# 德塔数据库源码，该数据的亮点为：一次执行多项复杂任务， 简易命令行 PLSQL 编程语言模型，打破现在所有传统关系数据库数据的单任务执行模式。基于双向队列的日志分析功能和命令语句执行方式和量子碎片文件存储思想，永远告别死锁。同时提供rest数据库操作接口和plsql命令行执行接口2种需求，满足各种场合的数据库操作应用。改数据库admin系统简历在deta VPCS HTTP服务器中， 整个系统启动时间 50毫秒。有效和运维交互。
...
###### 已完善功能：1 update rest接口涉及的缓存更新
###### 已完善功能：2 delete rest接口涉及的缓存更新
###### 已完善功能：3 insert rest接口涉及的缓存更新
